...
const threatRows = [
  ["Server privately tests shuffles before ever committing to one", "The server publishes a hash of its own seed for the next hand before that hand's player seeds are collected, and the hand ID is fixed at the same moment - so its contribution is locked in before it can see what anyone sent. The revealed seed is re-hashed against that commitment by verifyTableBundle", "Live, checked by verifyTableBundle", "pass"],
  ["Server re-rolls a seat's seed after seeing the others, by claiming that seat never sent one", "A seat that doesn't supply its own randomness gets a value derived deterministically from the already-committed server seed - not a fresh roll. verifyTableBundle recomputes every server-sourced seat's seed and rejects the receipt if it doesn't match, so \"they didn't send one in time\" can't be used as cover for a hand-picked value", "Live, checked by verifyTableBundle", "pass"],
  ["Server swaps in a different seed for a seat that did send one", "Every seed is acknowledged before the hand with a signed statement - \"for hand H, seat N, I hold a seed hashing to X\" - issued to that seat and kept in its browser. A receipt for hand H showing a different seed for seat N contradicts a signature the operator cannot disown, so this stops being one party's word against the other's. Verify against the key published at /api/fairness/public-key - and pin it from FAIRNESS-KEY.md in the public repo, since an operator free to rotate keys could otherwise disown its own signatures", "Live - signed acknowledgement, independently checkable by anyone", "pass"],
  ...
  ["Folded cards stay private after the hand (trustless tables)", "No - and this is the price of the receipt. Replaying the shuffle requires both players' masking seeds, and the finished receipt publishes them, so anyone holding it can decrypt every position - including the hole cards of a hand that was folded and never shown. Nobody can read them DURING the hand, which is the property the mode exists for; afterwards, everything is open. A hand folded before any card is turned up publishes no receipt at all, so it stays private", "Inherent to a verifiable shuffle - disclosed, not mitigated", "hard"],
      ...
      <main className="fairness-page">
        ...
        <section className="protocol-premise">
          ...
          <div className="premise-main"><h2>RIVER trusts the same server. It just makes that trust checkable.</h2><div className="premise-grid">
            ...
            <article><EyeOff size={20} /><span>DEALING</span><h3>Who sees each card?</h3><p>On a server-dealt table, hole cards go 1:1 to the owning connection and are never broadcast — but the server dealt them, so that&apos;s an operational guarantee, not a cryptographic one. On a trustless table it <em>is</em> cryptographic: the server holds no key and can&apos;t decrypt a card at all. The trade is that the receipt publishes both masking seeds afterwards, which is what lets anyone replay the shuffle - so once a hand is over, every card in it can be derived, including one that was folded and never shown.</p></article>
        ...
        <section className="architecture-section" id="architecture">
          ...
          <div className="architecture-flow">
            <article className="implemented"><div className="architecture-node"><KeyRound size={21} /><span>01</span></div><div><span>PRE-HAND</span><h3>Commit entropy</h3><p>The server publishes a hash of its own seed for the next hand before that hand&apos;s player seeds are collected, then every dealt-in seat&apos;s browser adds its own randomness. A seat that doesn&apos;t supply one in time falls back to a value derived from that already-published commitment - never a fresh roll taken after the server has seen what everyone else sent.</p><small><Check size={12} /> LIVE</small></div></article>
